...
typedef	std::map<int, Handler*> 	t_socketfd_handler_ptr_map;
...
//	TOOL -------------------------------------------------------------
	...
	private:
		...
		t_socketfd_handler_ptr_map		app_handlers_can_read_;
		t_socketfd_handler_ptr_map		app_handlers_prepare_response_;		
		t_socketfd_handler_ptr_map		app_handlers_can_response_;				
	private:
		int								epoll_fd_;
	private:
		int								epoll_count_;
	...
	private:
		...
		void									switch_to_prepare_response_cgi	(Handler* handler_ptr);
		void									delete_handler_from_map			(const int handler_socket_fd, t_socketfd_handler_map& handler_map);
		void									delete_handler_from_map			(const int handler_socket_fd, t_socketfd_handler_ptr_map& handler_map);		
	...
	private:
		...
		void								prepare_head						(Handler* handler_ptr);		
		void								prepare_post						(Handler* handler_ptr);
		void								prepare_put							(Handler* handler_ptr);
	private:
