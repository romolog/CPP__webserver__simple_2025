...
int		AppRun::send_response(const int handler_socket_fd)
{
	...
	Handler* handler_ptr = app_handlers_can_response_[handler_socket_fd];

	// MSG_DONTWAIT : 
	std::size_t response_len = handler_ptr->http_response_.response_str_.size();
	ssize_t check_send = send(handler_socket_fd, handler_ptr->http_response_.response_str_.c_str(), response_len, MSG_DONTWAIT);
	...
	if (handler_ptr->http_response_.is_chunked_ == NO_ || \
		(handler_ptr->http_response_.is_chunked_ == YES_ && handler_ptr->http_response_.is_final_chunk_ == YES_))
	{
		handler_ptr->reset();
	...
	else if (handler_ptr->http_response_.is_chunked_ == YES_)
	{
		handler_ptr->http_response_.response_str_ = "";
