...
void	AppRun::cgi_launch(Handler *handler_ptr)
{
	...
	bool is_exist = !access(target.c_str(), F_OK);	
	bool is_executable = is_exist && !access(target.c_str(), X_OK);
	...
	if (!is_exist || !is_regular_file || !is_executable || file_err)
	{
		std::string err_msg = "GET: Invalid required resource " + \
								target.substr(0, ERROR_INPUT_SNIPET_LEN_) + ft_code_location(__FILE__, __LINE__);
		set_error_response(handler_ptr, err_msg, 500);
	...
	else if (cgi_pid == CHILD)
	{
		...
		// close fds
		(void)close(handler_ptr->socket_fd_);
		...
		if (check_close == -1)
		{
			...
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed close cgi_sockets[0]: " << strerror(errno) << " : " \
		...
		if (check_chdir == -1)
		{
			...
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed chdir: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}

		target = "./" + target.substr(target_dir.size());
		...
		if (check_dup2 == -1)
		{
			...
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed dup2 STDOUT: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}
		...
		if (check_write == -1 || check_write != static_cast<int>(body_size))
		{
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed write body to socket: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}		
		...
		if (check_dup2 == -1)
		{
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_\
						<< "CGI failed dup2 STDIN: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}
		...
		if (check_close == -1)
		{
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed close cgi_sockets_in[0]: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}
		...
		if (check_close == -1)
		{
			#ifndef NDEBUG
			std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI failed close cgi_sockets_in[1]: " << strerror(errno) << " : " \
						<< ft_code_location(__FILE__, __LINE__) << '\n';
			#endif
			exit(EXIT_FAILURE);
		}
		...
		//	to prevent invalidation of pointer;
		std::string cgi_path(target);
		const char* cgi_path_ptr = cgi_path.c_str();

		std::string cgi_name(target);
		const char* temp_const_char = cgi_name.c_str();
		...
		execve(cgi_path_ptr, args, cgi_request.cgi_env_);


		// PARENT will catch EXIT_FAILURE and set ERROR_RESPONSE
		#ifndef NDEBUG
		std::cerr	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
					<< "CGI failed execve: " << strerror(errno) << " : " \
					<< ft_code_location(__FILE__, __LINE__) << '\n';
		#endif
		exit(EXIT_FAILURE);
	}
	else // PARENT:
	{
		...
		{
			std::cerr 	<< TAB_ << prog_name_ << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_ \
						<< "CGI parent: System error: Failed close socket: " + \
							ft_itoa(cgi_sockets_out[1]) + " : " + \
							ft_code_location(__FILE__, __LINE__) << '\n';
		}
		#else
			(void)check_close;
