...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   exec_AppRun_70_prepare_delete.cpp                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
...
void	AppRun::prepare_delete(Handler *handler_ptr)
{
	...
	if (!is_exist) // target doesn't exist
	{
		set_error_response(handler_ptr, "DELETE: Target doesn't exist" + ft_code_location(__FILE__, __LINE__), 404);
		prepare_error(handler_ptr);
		return ;
	}
	else // target exists
	{
		int check_delete = std::remove(target.c_str());
		if (check_delete == -1)
		{
			set_error_response(handler_ptr, "DELETE: Can't delete file" + ft_code_location(__FILE__, __LINE__), 500);
			prepare_error(handler_ptr);
			return ;
		}
	}
