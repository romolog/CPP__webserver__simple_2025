...
int	main(int argc, char *argv[], char** env)
{
	...
	try
	{
		//	argc > 2 exit(failure), argc = 2 config file = argv[1], argc = 1 config file = default
		ConfigSwitch config_switch(argc, argv[1], prog_name);
		std::string& config_file = config_switch.config_file_name_;
	...
	{
		std::cerr 	<< TAB_ << prog_name << RED_BOLD_HIGH_ << ERROR_ << DEFAULT_COLOR_  \
					<< "System error: " << e.what() << '\n';
		return (EXIT_FAILURE);
	}
	catch (...)
